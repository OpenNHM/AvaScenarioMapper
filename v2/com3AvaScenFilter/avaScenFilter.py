...
#       - Deduplication : enforce a single logical avalanche result per scenario,
#                         keeping the result with the largest relative size (rSize)
#                         while preserving the rel/res pair via (praID, resultID)
#
# Output :
...
def filterScenarioResults(
    ...
    def _pickWinningPairsByRsize(df: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        """
        Keep the winning (praID, resultID) pair per PRA/scenario context,
        preferring the highest rSize and preserving both rel/res rows.
        """
        if df is None or df.empty:
            return df

        missingPairCols = [c for c in PAIR_KEYS if c not in df.columns]
        if missingPairCols:
            log.warning(
                "Single-rSize rule requested, but pair key columns missing: %s. "
                "Skipping pair-safe deduplication.",
                ", ".join(missingPairCols),
            )
            return df

        work = df.copy()

        if "rSize" in work.columns:
            work["rSize"] = pd.to_numeric(work["rSize"], errors="coerce")

        pairCols = [c for c in PAIR_KEYS if c in work.columns]
        pairInfoCols = [c for c in ["rSize"] if c in work.columns]

        pairDf = work[pairCols + pairInfoCols].drop_duplicates(subset=pairCols).copy()
        ...
        groupCols = [c for c in groupCols if c in work.columns]

        pairContext = work[groupCols + pairCols].drop_duplicates(subset=pairCols).copy()
        pairDf = pairContext.merge(pairDf, on=pairCols, how="left")

        sortCols = []
        ascending = []

        if "rSize" in pairDf.columns:
            sortCols.append("rSize")
            ascending.append(False)

        if "resultID" in pairDf.columns:
            sortCols.append("resultID")
            ascending.append(True)

        if sortCols:
            pairDf = pairDf.sort_values(sortCols, ascending=ascending)

        keepPairs = pairDf.drop_duplicates(subset=groupCols, keep="first")[pairCols].copy()

        before = len(work)
        deduped = work.merge(keepPairs, on=pairCols, how="inner")

        log.info(
            "Applied pair-safe single-rSize rule: kept %d/%d rows across %d winning pair(s)",
            len(deduped),
            before,
            len(keepPairs),
        )
    ...
    # IMPORTANT: keep original attribute names
    ...
    if avaDistPot and avaSizePot is not None:
        ...
        if sizeRef == 1:
            ...
            relOnly = _pickWinningPairsByRsize(relOnly)
            _logStage("after_legend", relOnly)
            gdf = relOnly
    ...
    if applySingle and not gdf.empty:
        gdf = _pickWinningPairsByRsize(gdf)
